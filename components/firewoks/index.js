...
const SPARK_NUM = 60 //当作烟花炸开的数量
const A=8;      //考虑到空气阻力，重力加速度设为8
...
export default class Firework {
  ...
  getSparks() {
    ...
    if(this.count%INTERVAL==1){
      if(this.boomState==0){
        ...
        res= this.sparks.map(item=>new Point(item.position,this.color,item.size));
        console.log(res);
      }
    }
  ...
  createSparks(){
    ...
    let ct=2000/DURATION;
    for(let j=0;j<circle;j++){
      for(let i=0;i<sPC;i++){
        let rand=Math.random()/2.5+0.8; //0.8-1.2
        let ancle=perAngle*j*rand;
        this.sparks.push({
          size:this.size,
          speed:{
            x:SPEED*Math.sin(ancle),
            y:SPEED*Math.cos(ancle)
          },
